...
tmux new-session -d -s "${session}" -n "PX4-1" \
	"${PX4_ENV_PREFIX}cd \"${PX4_DIR}\" && unset PX4_GZ_STANDALONE; PX4_SYS_AUTOSTART=4010 PX4_SYS_ID=1 PX4_GZ_MODEL_POSE=\"-7,5,0.5\" PX4_SIM_MODEL=gz_x500_mono_cam \"${PX4_BIN}\" -i 1"

sleep 12

tmux new-window -t "${session}:" -n "PX4-2" \
	"${PX4_ENV_PREFIX}cd \"${PX4_DIR}\" && PX4_GZ_STANDALONE=1 PX4_SYS_AUTOSTART=4001 PX4_SYS_ID=2 PX4_GZ_MODEL_POSE=\"-7,4,0.5\" PX4_SIM_MODEL=gz_x500_mono_cam_down \"${PX4_BIN}\" -i 2"

tmux new-window -t "${session}:" -n "PX4-3" \
	"${PX4_ENV_PREFIX}cd \"${PX4_DIR}\" && PX4_GZ_STANDALONE=1 PX4_SYS_AUTOSTART=4001 PX4_SYS_ID=3 PX4_GZ_MODEL_POSE=\"-7,6,0.5\" PX4_SIM_MODEL=gz_x500_mono_cam_down \"${PX4_BIN}\" -i 3"
